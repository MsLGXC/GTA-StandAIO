...
menu.action(menu.my_root(), "汽水罐", {"sprunkdrop"}, "在你附近掉落一罐汽水", function()
    sprunk_can_drop(players.get_position(players.user()))
end)

menu.toggle_loop(menu.my_root(), "汽水暴雨", {"sprunkrain"}, "在你附近下起汽水暴雨", function()
    sprunk_raindrop_player(players.user())
    util.yield(config.can_rain_delay)
end)

menu.action(menu.my_root(), "绿汽水", {}, "好绿的汽水", function(click_type, pid)
...
menu.action(menu.my_root(), "绿车[先进载具]", {"sprunkify"}, "你的绿车!", function(click_type, pid)
...
menu.action(menu.my_root(), "倾倒垃圾罐", {"ecola"}, "离离原上草!", function(click_type)
    local pid = players.user()
    trash_dump_player(pid)
end, nil, nil, COMMANDPERM_FRIENDLY)

menu.action(menu.my_root(), "绿气传染", {}, "把你的绿色传染给附近载具!", function(click_type)
...
menu.toggle(menu.my_root(), "幸福的飞艇", {}, "快抬头看看!", function(on)
...
menu.toggle_loop(menu.my_root(), "循环购买", {}, "站在汽水贩卖机前循环开始购买你的绿汽水吧！哎~咋买的是红色的！'", function()
...
local script_meta_menu = menu.list(menu.my_root(), "脚本信息")
