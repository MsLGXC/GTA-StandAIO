...
local fail_text = "此文件未能正确加载，可能是不受支持的格式。请提交一份错误报告至Discord，并附上您刚刚尝试加载的载具。"
local instruction_text = "\n按住shift键和ctrl键的同时单击一个选项以从目录中删除此载具.\n按住空格的同时单击一个选项以添加到收藏夹."
...
if filesystem.is_dir(filesystem.scripts_dir() .. '\\menyoo vehicles') or filesystem.is_dir(filesystem.scripts_dir() .. '\\menyoo maps') and not SCRIPT_SILENT_START then
    util.toast("你有旧的menyoo载具目录！事情发生了变化。")
    util.toast("将载具和地图放入Lua Scripts / store / LanceSpooner / vehicles 或 maps,以便能够生成它们:).删除旧目录来移除此消息。")
end
...
local load_vehicle_action = menu.list_action(menu.my_root(), "加载载具", {}, "加载一辆载具. " .. instruction_text, all_vehicles, function(index, value)
...
loaded_vehicles_root = menu.list(menu.my_root(), "当前加载的载具", {"lancespoonerloadedvehicles"}, "您已生成的载具")
...
local load_map_action = menu.list_action(menu.my_root(), "加载地图", {}, "加载一个地图. " .. instruction_text, all_maps, function(index, value)
...
loaded_maps_root = menu.list(menu.my_root(), "当前已加载的地图", {"lancespoonerloadedmaps"}, "您已生成的地图")


menu.divider(menu.my_root(), "收藏夹")

local load_favorite_vehicle_action = menu.list_action(menu.my_root(), "最喜爱的载具", {}, "加载一辆载具", favorite_vehicles, function(index, value)
    load_vehicle(vehicles_dir .. '\\' .. value)
end)

local load_favorite_map_action = menu.list_action(menu.my_root(), "最喜爱的地图", {}, "加载一张地图", favorite_maps, function(index, value)
...
local disp_version = string.gsub(json_tbl.version, 'Jackz Custom Vehicle ', '')
    if not table.contains(supported_jackz_versions, disp_version) then 
        util.toast("LanceSpooner不支持此Jackz JSON版本(版本 " .. disp_version .. "). 无论如何我们都会尝试加载它，但有些东西可能无法工作。如果它确实有效，请告诉我们，这样我们可以少显示此消息！")
    end
else
    util.toast("这是一辆很旧的Jackz载具，可能不受支持（或者这根本不是Jackz载具）。事情可能会被弄坏，但无论如何我们都会设法加载它.")
end
