...
if not filesystem.is_dir(store_dir) then
    ...
    util.toast("欢迎来到lance创造器!")
    util.toast("将您的载具和地图放入Lua Scripts / store / LanceSpooner / vehicles 或 maps:)")
end
end

if filesystem.is_dir(filesystem.scripts_dir() .. '\\menyoo vehicles') or filesystem.is_dir(filesystem.scripts_dir() .. '\\menyoo maps') then
    util.toast("你有旧的menyoo车辆目录！事情发生了变化。")
    util.toast("将车辆和地图放入Lua Scripts / store / LanceSpooner / vehicles 或 maps,以便能够生成它们:).删除旧目录来移除此消息。")
end
...
menu.action(menu.my_root(), "搜索载具", {"lssearchv"}, "", function(click_type)
...
local load_menyoo_vehicle_action = menu.list_action(menu.my_root(), "加载XML载具", {}, "加载一辆载具。按住shift键和ctrl键的同时单击一个选项以从目录中删除此载具。", all_menyoo_vehicles, function(index, value)
...
local load_ini_vehicle_action = menu.list_action(menu.my_root(), "加载INI载具", {}, "加载一辆载具。按住shift键和ctrl键的同时单击一个选项以从目录中删除此载具。", all_ini_vehicles, function(index, value)
    local path = vehicles_dir .. '\\' .. value
    if util.is_key_down(0x10) and util.is_key_down(0x11) then
        os.remove(path)
        util.toast(value .. " 已删除! :)")
...
local load_json_vehicle_action = menu.list_action(menu.my_root(), "加载JSON/Jackz载具", {}, "加载一辆载具。按住shift键和ctrl键的同时单击一个选项以从目录中删除此载具。", all_ini_vehicles, function(index, value)
    local path = vehicles_dir .. '\\' .. value
    if util.is_key_down(0x10) and util.is_key_down(0x11) then
        os.remove(path)
        util.toast(value .. " 已删除! :)")
...
menu.action(menu.my_root(), "搜索地图", {"lssearchmap"}, "", function(click_type)
...
local load_menyoo_map_action = menu.list_action(menu.my_root(), "加载XML地图", {}, "加载一个地图。按住shift键和ctrl键的同时单击一个选项以从目录中删除此载具。", all_menyoo_maps, function(index, value)
...
loaded_maps_root = menu.list(menu.my_root(), "当前加载的地图", {"lancespoonerloadedmaps"}, "已加载的地图")
loaded_vehicles_root = menu.list(menu.my_root(), "当前加载的载具", {"lancespoonerloadedvehicles"}, "你生成的载具")
...
async_http.init("pastebin.com", "/raw/nrMdhHwE", function(result)
    menu.hyperlink(menu.my_root(), "加入 discord-原作者xSetrox", result, "")
end)
async_http.dispatch()

async_http.init("pastebin.com", "/raw/HScMyPPf", function(result)
    menu.hyperlink(menu.my_root(), "加入qq群-汉化者MrLGXC", result, "")
end)
async_http.dispatch()
...
function ini_load_vehicle(file_name)
    ...
    if not success then 
        util.toast("尝试加载此INI时发生错误!请检查INI文件并重试。")
    end
    local all_entities = {}
    local ini_type = -1
...
local disp_version = string.gsub(json_tbl.version, 'Jackz Custom Vehicle ', '')
    if not table.contains(supported_jackz_versions, disp_version) then 
        util.toast("LanceSpooner不支持此Jackz JSON版本(版本 " .. disp_version .. "). 有些事情可能不起作用。如果确实有效，请告诉我们，这样我们就可以少显示此消息！")
    end
else
