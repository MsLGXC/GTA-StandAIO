...
local bigvehiclenames = {"titan", "bombushka", "volatol", "alkonost", "kosatka", "avenger", "avenger2", "jet", "cargoplane", "blimp", "blimp2", "blimp3"}
local bigvehicle = {}
...
local function does_entity_from_pointer_exist(addr)
    if allentities[addr] then
        return true
    end
    return false
end
local function network_network_get_network_id_from_entity(Entity)
    native_invoker.begin_call()
    native_invoker.push_arg_int(Entity)
    native_invoker.end_call_2(0xA11700682F3AD45C)
...
local function is_high_priority_entity(addr)
    local NetObj = memory.read_long(addr + 0xD0)
    return ((NetObj and NetObj ~= 0) and (memory.read_ubyte(NetObj + 0x4E) ~= 0)) or (network_network_get_network_id_from_entity(Entity) ~= 0)
end
...
local autothrottler = menu.list(menu.my_root(), "自动节流", {}, "")
    ...
    for i = 0, 31 do
        if players.exists(i) then
            ...
            if #whitelist.auto[i].current > settings.auto.totallimit then
                notification("[T] 节流实体来自 "..tostring(players.get_name(i)), TOAST_ALL)
            ...
            for Model, Num in pairs(whitelist.auto[i].models) do
                if Num > settings.auto.modellimit then
                    notification("[M] 节流实体来自 "..tostring(players.get_name(i)), TOAST_ALL)
                    if util.current_time_millis() + settings.auto.timeout > synctimer[i] then
                        synctimer[i] = util.current_time_millis() + settings.auto.timeout
                    end
                    if settings.auto.cleanup then
            ...
            if whitelist.auto[i].highpriority.total > settings.auto.highlimit then
                notification("[H] 节流实体来自 "..tostring(players.get_name(i)), TOAST_ALL)
                if util.current_time_millis() + settings.auto.timeout > synctimer[i] then
                    synctimer[i] = util.current_time_millis() + settings.auto.timeout
                end
                if settings.auto.cleanup then
...
local bigthrottler = menu.list(menu.my_root(), "大型载具节流", {}, "")
...
menu.toggle_loop(bigthrottler, "启用", {}, "", function()
    ...
    if #Pointers > settings.bigvehicle.limit then
        ...
        if players.exists(owner) then
            notification("节流大型载具来自 "..tostring(players.get_name(owner)), TOAST_ALL)
...
menu.slider(bigthrottler, "间隔 (秒)", {}, "指定(秒)实体将持续多久时间才会被认定为垃圾实体.", 0, 60, settings.bigvehicle.timeout/1000, 1, function(value)
...
local vehiclethrottler = menu.list(menu.my_root(), "载具节流", {}, "")
...
menu.toggle_loop(vehiclethrottler, "启用", {}, "", function()
    ...
    if #Pointers > settings.vehicle.limit then
        ...
        local owner = get_highest_count(owners)
        if players.exists(owner) then
            notification("节流载具来自 "..tostring(players.get_name(owner)), TOAST_ALL)
...
local objectthrottler = menu.list(menu.my_root(), "物体节流", {}, "")
...
menu.toggle_loop(objectthrottler, "启用", {}, "", function()
    ...
    if #Pointers > settings.object.limit then
        ...
        local owner = get_highest_count(owners)
        if players.exists(owner) then
            notification("节流物体来自 "..tostring(players.get_name(owner)), TOAST_ALL)
...
local pedthrottler = menu.list(menu.my_root(), "Ped节流", {}, "")
...
menu.toggle_loop(pedthrottler, "启用", {}, "", function()
    ...
    if #Pointers > settings.ped.limit then
        ...
        local owner = get_highest_count(owners)
        if players.exists(owner) then
            notification("节流Ped来自 "..tostring(players.get_name(owner)), TOAST_ALL)
